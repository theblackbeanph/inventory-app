...
- **Cross-day gap (known limitation)**: both `cancelDispute` and `EditReceiveView` write compensating `branch_adjustments` dated `todayPHT()` (midnight flip, not `businessDatePHT`) and never mutate the original `"commissary transfer"` IN adj. If the dispute crosses a submitted `daily_close`, the branch's net `branch_stock` reconciles but the original date's IN column and `daily_close.items[<item>]` snapshot stay wrong. Recovery is a manual patch to BOTH: (1) the adj doc (`qty` field), and (2) `daily_close.items.<item>` (rewrite the whole entry — beginning/inQty/outQty/expected/endCount/variance — since `daily_close.items.outQty` may also be stale vs live if the 1am `sales_import` cron landed after close). See `scripts/patch-burger-patty-0803-in.mjs` for the pattern. Verified 2026-08-04 during the PO-26-0803-MKT001 smoke test.
