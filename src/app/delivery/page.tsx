      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 20, fontWeight: 700 }}>Delivery</div>
          </div>
          {inTransitCount > 0 && (
          ...
          return (
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                ...
                }}>{STATUS_LABEL[note.status]}</span>
              </div>
              <div style={{ fontSize: 13, color: "var(--text-secondary)", marginBottom: 4 }}>
                {note.po_number} · {note.items.length} items
              </div>
              {canConfirm && (
              ...
              {note.has_discrepancy && (
                <div style={{ fontSize: 12, color: "#DC2626", fontWeight: 600 }}>
                  Discrepancy reported — commissary notified
                </div>
              )}
            </div>
          );
...
// ── Delivery Detail / Receipt Confirmation ────────────────────────────────────
  ...
  async function confirmReceipt() {
    ...
    const updatedItems: DeliveryNoteItem[] = note.items.map(i => {
      ...
      return {
        ...
        discrepancy: received - i.dispatched_qty,
      };
    });
    ...
    // Write discrepancy entries to commissary's invEntries collection
    if (hasDiscrepancy) {
      for (const item of updatedItems) {
        ...
        saves.push(saveDocById(COLS.invEntries, entryId, entry));
      }
    }
        ...
        {note.items.map(item => {
          ...
          return (
            <div key={item.item_name} style={{
              ...
              borderLeft: isDiff ? "4px solid #DC2626" : "4px solid transparent",
            }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                {canConfirm ? (
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <button
                      onClick={() => setReceivedQtys(p => ({ ...p, [item.item_name]: Math.max(0, (p[item.item_name] ?? item.dispatched_qty) - 1) }))}
                      style={qtyBtnStyle}
                    >−</button>
                    <input
                      type="number"
                      ...
                      style={{
                        width: 56, textAlign: "center", border: `1.5px solid ${isDiff ? "#DC2626" : "var(--border)"}`,
                        borderRadius: 8, padding: "6px 4px", fontSize: 16, fontWeight: 700,
                        background: isDiff ? "#FEF2F2" : "var(--bg)", color: "var(--text)",
                      }}
                    />
                    <button
                      onClick={() => setReceivedQtys(p => ({ ...p, [item.item_name]: (p[item.item_name] ?? item.dispatched_qty) + 1 }))}
                      style={qtyBtnStyle}
                    >+</button>
                  </div>
                ) : (
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontWeight: 700, fontSize: 18, color: isDiff ? "#DC2626" : "var(--text)" }}>
            ...
            <>
              ...
              <textarea
                ...
                style={{
                  ...
                  background: "#FEF2F2", color: "var(--text)", boxSizing: "border-box", marginBottom: 8,
                }}
              />
            </>
          )}
          <button
            onClick={confirmReceipt} disabled={loading}
            style={{
              width: "100%", padding: "15px 0", borderRadius: 14, border: "none",
              background: hasDiscrepancy ? "#DC2626" : "#059669",
