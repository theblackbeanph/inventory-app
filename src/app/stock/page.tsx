...
export default function StockPage() {
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "var(--bg)", paddingBottom: "calc(var(--nav-h) + 16px)" }}>
      ...
      {showStoreHubSync && branch && department && (
        <StoreHubSyncModal
          branch={branch}
...
// ── StoreHub Sync Modal (MKT only) ────────────────────────────────────────────

function StoreHubSyncModal({ branch, department, today, onClose }: { branch: Branch; department: Department; today: string; onClose: () => void }) {
          ...
          <>
            ...
            {unmatchedSkus.length > 0 && (
              <div style={{ background: "#FEF3C7", borderRadius: 10, padding: "10px 14px", marginBottom: 20 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#D97706", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>Not tracked — {unmatchedSkus.length} SKUs</div>
                {unmatchedSkus.map(({ sku, qty }) => (
                  <div key={sku} style={{ fontSize: 12, color: "#92400E", marginBottom: 2 }}>· {sku} ({qty} sold)</div>
                ))}
              </div>
            )}
